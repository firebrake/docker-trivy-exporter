...
RUN set -ex; \
    ...
    curl -sSL https://github.com/aquasecurity/trivy/releases/download/v${TRIVY_VERSION}/trivy_${TRIVY_VERSION}_Linux-64bit.tar.gz -o /root/trivy.tar.gz; \
...
ENV TRIVY_VERSION=${TRIVY_VERSION} \
    ...
    TRIVY_EXPORTER_IMAGES_SCANNERS=config,license,secret,vuln \
    TRIVY_EXPORTER_ROOTFS_PATH=/rootfs \
    TRIVY_EXPORTER_ROOTFS_SCAN=1 \
    TRIVY_EXPORTER_ROOTFS_SCANNERS=config,license,secret,vuln
